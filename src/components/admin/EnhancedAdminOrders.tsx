...
const EnhancedAdminOrders = () => {
  ...
  const orderStats = {
    ...
    totalProfit: orders.reduce((sum, order) => sum + parseFloat(order.profit || 0), 0)
  };
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      {order.deposit > 0 && (
                        <div className="text-xs text-blue-600">
                          <strong>المبلغ المسدد:</strong> {formatCurrency(order.deposit)} • 
                          <strong> المتبقي:</strong> {formatCurrency(order.total_amount - order.deposit)}
                        </div>
                      )}
                    </div>
